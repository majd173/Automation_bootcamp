...
class Pet:
    ...
    def __init__(self, name, species, age, owner: Owner, vaccinated, total_pets):
        self.name = name
    ...
    def __eq__(self, other):
        try:
            ...
            logging.error("Error checking if pets are equal: {e}")

    def add_pet(self, pet: 'Pet'):
        try:
            logging.info("Adding a new pet")
            self.total_pets.append(pet)
            return self.total_pets
        except Exception as e:
    ...
    def total_pets_count(self):
        try:
            ...
            total_pets = len(self.total_pets)
            return total_pets
        except Exception as e:
    ...
    def save_store(self, pet_file_path):
        with open(pet_file_path, 'w') as file:
            json.dump({'name': self.name, 'species': self.species,
                       'age': self.age, 'owner': self.owner, 'vaccinated': self._vaccinated}, file, indent=1)
